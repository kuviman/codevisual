    ...
    Lib: ShaderLibrary,
{
    ...
    ugli::Program::new(ugli_context, &[&vertex_shader, &fragment_shader])
...
where
    ...
    D: ShaderDefineStorage + Clone + PartialEq,
{
    pub fn new(context: &Rc<ugli::Context>, uniforms: U, defines: D, program_source: &str) -> Self {
        let program_source = String::from(program_source);
        let last_defines = RefCell::new(defines.clone());
        let program = RefCell::new(compile_ugli_program::<Lib>(
            context,
            &defines,
            &program_source,
        ));
    ...
    pub fn ugli_program(&self) -> Ref<ugli::Program> {
        if *self.last_defines.borrow() != self.defines {
            ...
            *self.program.borrow_mut() = compile_ugli_program::<Lib>(
                &self.ugli_context,
                &self.defines,
                &self.program_source,
            );
