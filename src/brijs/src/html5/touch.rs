...
pub fn set_touchstart_callback<F: FnMut(TouchStartEvent)>(callback: F) {
    ...
    unsafe {
        emscripten_set_touchstart_callback(CANVAS_SELECTOR.as_ptr(),
...
pub fn set_touchmove_callback<F: FnMut(TouchMoveEvent)>(callback: F) {
    let callback = Box::new(Box::new(callback));
    unsafe {
        emscripten_set_touchmove_callback(CANVAS_SELECTOR.as_ptr(),
                                          Box::into_raw(callback) as *mut _,
                                          USE_CAPTURE,
                                          Some(wrapper::<F>));
    }
...
pub fn set_touchend_callback<F: FnMut(TouchEndEvent)>(callback: F) {
    let callback = Box::new(Box::new(callback));
    unsafe {
        emscripten_set_touchend_callback(CANVAS_SELECTOR.as_ptr(),
                                         Box::into_raw(callback) as *mut _,
                                         USE_CAPTURE,
                                         Some(wrapper::<F>));
    }
